...
fun CPKIdentifier.toCorda() =
    CPK.Identifier.newInstance(
        name,
        version,
        net.corda.v5.crypto.SecureHash(signerSummaryHash.algorithm, signerSummaryHash.serverHash.array()))
...
fun HoldingIdentity.toCorda() = CPI.Identity.newInstance(X500Principal(x500Name), groupId)

fun CPI.Identity.toAvro() = HoldingIdentity.newBuilder().also {
    it.x500Name = name.toString()
    it.groupId = groupId
}.build()
...
fun CPI.Identifier.toAvro() = CPIIdentifier.newBuilder().also {
    ...
    it.identity = identity?.toAvro()
}.build()

fun CPIMetadata.toCorda() = CPI.Metadata.newInstance(id.toCorda(), cpks.map(CPKMetadata::toCorda), networkPolicy)
